...
my-lc adds two verbs of its own, which wrap nothing: `install` and
`uninstall`, for the run recorder described below.
...
STATUS answers what happened to the **run**. What would start it next is the
TRIGGER column's answer, and saying it twice told you nothing:

| | |
|---|---|
| `RUNNING SINCE:2026-07-06_1216 [58d5h] pid 864` | with its uptime |
| `OK LAST:2026-09-02_1732[0]` | the raw exit code beside the word derived from it |
...
- **`LAST:`** — a real run time, from the run recorder below
- **`LAST~:`** — *reasoned, not observed*: a boot-triggered service that has
  run, ran when it was loaded; a calendar job ran at its previous occurrence.
  Suppressed when the evidence contradicts it
...
launchd records **no run times at all**. `launchctl print` gives a run
*count* and the last exit code, nothing dated, and all of it is per load: a
`bootout` discards even the count. The unified log has the times, but reading
them on demand costs ~2.3s for a five-minute window and the store only
...
One root daemon serves every domain: reading the log is admin-only, so a
per-user agent could not do it — but launchd's events name their domain, so
each user's records land in that user's own directory.
